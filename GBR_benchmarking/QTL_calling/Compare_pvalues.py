...
def readin_one_chromosome(QTL_dir, file_suffix):
	...
	fn1 = '%s/CHR%_%s.txt' % (QTL_dir, file_suffix)
...
def readin_both_results(QTL_dir, chromsome, window, suffix):
    ...
    [call_QTL_imputed, call_QTL_imputed_sig] = readin_one_chromosome('%s/Imputation' % QTL_dir, 'caQTLs_WINDOW_%skb%s_withImputation%s.txt' % (str(window/1000.0), suffix)
...
def obtain_metrics(peak_calling, window, suffix = '_noWeight'):
    ...
    r2_caller = np.corrcoef(-np.log10(both_tested['P-value']), -np.log10(both_tested['Pvalue_genotype_caller']))

    # imputation
    both_tested = df[(~df['Pvalue_imputation'].isnull()) & (~df['P-value'].isnull())]
    r2_imputation = np.corrcoef(-np.log10(both_tested['P-value']), -np.log10(both_tested['Pvalue_imputation']))
    ...
    set1 = set(df_sig[~df_sig['P-value'].isnull()].apply(lambda x: '_'.join((x['CHR_POS'], x['PeakID'])), axis=1))
    set2 = set(df_sig[~df_sig['Pvalue_genotype_caller'].isnull()].apply(lambda x: '_'.join((x['CHR_POS'], x['PeakID'])), axis=1))
    set3 = set(df_sig[~df_sig['Pvalue_imputation'].isnull()].apply(lambda x: '_'.join((x['CHR_POS'], x['PeakID'])), axis=1))

    precision_caller = np.intersect1d(set1, set2) / len(set2)
    recall_caller = np.intersect1d(set1, set2) / len(set1)
    precision_imputation = np.intersect1d(set1, set3) / len(set3)
    recall_imputation = np.intersect1d(set1, set3) / len(set1)

    result = [r2_caller, precision_caller, recall_caller, r2_imputation, precision_imputation, recall_imputation]
