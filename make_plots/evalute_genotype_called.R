...
g_recovered = ggplot() + 
  ...
  ylab('Fraction of variants from genotype data that are called') + 
  xlab('Samples') + 
  scale_fill_brewer(palette = 'Set1') + 
  guides(fill=guide_legend(title="")) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 90))
ggsave("results/Number_recovered_variants_with_Imputation.png", g_recovered, height = 4, width = 12)
...
metrics = metrics[metrics$group %in% c("performance_in_original", "performance_in_imputed", "performance_overall", "performance_overall_withPP"), ]
metrics$group = factor(metrics$group,  levels = c("performance_in_original", "performance_in_imputed", "performance_overall", "performance_overall_withPP"))
...
g_metrics = ggplot(data = metrics, aes(x = group, y=value, fill = group)) + 
  ...
  theme_bw() + 
  theme(axis.text.x = element_blank())
ggsave("results/Genotype_performance_with_Imputation_withPP.png", g_metrics, height = 4, width = 7)
