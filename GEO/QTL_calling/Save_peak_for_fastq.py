...
if __name__ == '__main__':
    ...
    [_, pca_model] = remove_PCs(centered_data)

    permuted_centered_data = read_in_peaks(PEAK_DIR, permute = True)
    [_, permuted_pca_model] = remove_PCs(permuted_centered_data)


    print(pca_model.explained_variance_ratio_)
    print(permuted_pca_model.explained_variance_ratio_)

    if np.sum(pca_model.explained_variance_ratio_ < permuted_pca_model.explained_variance_ratio_) > 0:
        idx = np.where( pca_model.explained_variance_ratio_ <  permuted_pca_model.explained_variance_ratio_)[0][0]
        print('Correct for %d PCs' % idx)
        [corrected_data, pca_model] = remove_PCs(centered_data, PCA_k=idx - 1)
    else:
