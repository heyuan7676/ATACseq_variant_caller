...
# shuffle samples in genotype
vcf_dat = as.data.frame(vcf_dat)
colnames(vcf_dat) = c(colnames(vcf_dat)[1:9], sample(samples))
vcf_dat = tibble(vcf_dat)
...
write.table(result, paste0(root_dir, 'mixLL/', 'chr',chromosome,'.window', cisDis,'.mixLL.r2.results.permuted.txt'), row.names=F, quote = F)
