...
awk 'NR==FNR{inFileA[$1]; next} ($1 in inFileA)' ${random_file} ${cadd_file} > ${qtl_dir}/cadd/${qtl_filename}.randomMatched.cadd.txt
...
## ENCODE
mkdir -p ${qtl_dir}/roadmap
dnase_enhancer=/work-zfs/abattle4/lab_data/roadmap/dnase_enhancer/all_peaks.tsv.gz
bedtools intersect  -a ${qtl_file}_sorted -b ${dnase_enhancer} -wb >  ${qtl_dir}/roadmap/${qtl_filename}.dnase_enhancer.txt
bedtools intersect  -a ${random_file}_sorted -b ${dnase_enhancer} -wb >  ${qtl_dir}/roadmap/${qtl_filename}.randomMatched.dnase_enhancer.txt


## promoter
dnase_promoter=/work-zfs/abattle4/lab_data/roadmap/dnase_promoter/all_peaks.tsv.gz
bedtools intersect  -a ${qtl_file}_sorted -b ${dnase_promoter} -wb >  ${qtl_dir}/roadmap/${qtl_filename}.dnase_promoter.txt
bedtools intersect  -a ${random_file}_sorted -b ${dnase_promoter} -wb >  ${qtl_dir}/roadmap/${qtl_filename}.randomMatched.dnase_promoter.txt
