...
Zenodo_DIR = os.path.join(PEAK_DIR, 'Zenodo')
DIR_FIRST_PASS = os.path.join(BOWTIE_DIR, 'first_pass_bqsr')

PEAK_UNION_fn = os.path.join(Zenodo_DIR, 'union-peaks.bed')
...
rule all:
    input:
        expand(os.path.join(Zenodo_DIR, '{indiv}.count.unionPeaks.bed'), indiv = INDIVS),
        expand(os.path.join(Zenodo_DIR, '{indiv}.count.unionPeaks.bed_matrix'), indiv = INDIVS),
        expand(os.path.join(Zenodo_DIR, "peak_by_sample_matrix_chr{chr}.txt"), chr = CHROM)
...
rule peak_IDs:
    input:
        ...
        peaks = os.path.join(Zenodo_DIR, 'peaks_Zenodo_loc_GRCh38.bed')
    output:
        union = PEAK_UNION_fn
    shell:
        """
...
rule count_reads:
    input:
        ...
        union = PEAK_UNION_fn
    output:
        os.path.join(Zenodo_DIR, '{indiv}.count.unionPeaks.bed')
    shell:
        """
...
rule collect_peak_union:
    input:
        readscount = os.path.join(Zenodo_DIR, '{indiv}.count.unionPeaks.bed'),
        peaks = os.path.join(Zenodo_DIR, PEAK_UNION_fn)
    output:
        os.path.join(Zenodo_DIR, '{indiv}.count.unionPeaks.bed_matrix')
    shell:
        """
...
rule obtain_peak_matrix:
    input:
        readscount = expand(os.path.join(Zenodo_DIR, '{indiv}.count.unionPeaks.bed_matrix'), indiv = INDIVS),
        peaks = PEAK_UNION_fn
    output:
        bychr = os.path.join(Zenodo_DIR, "peak_by_sample_matrix_chr{chr}.txt")
    shell:
        """
