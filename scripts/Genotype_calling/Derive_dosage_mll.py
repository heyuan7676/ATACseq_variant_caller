...
if __name__ == '__main__':
    ...
    filename = '%s/%s.filtered.minDP%d.recode.vcf.gz' % (VCF_dir, sample, minDP)
    ...
    dosage_with_ll['CHR_POS'] = ['_'.join(map(str, x)) for x in zip(np.array(pp_dat[0]), np.array(pp_dat[1]))]
    dosage_with_ll = dosage_with_ll[['CHR_POS', 'Dosage', 'GT']]
    dosage_with_ll.to_csv('%s/%s.filtered.minDP%d.recode.dosage_genotype.bed' % (VCF_dir, sample, minDP), sep='\t', index = False)
