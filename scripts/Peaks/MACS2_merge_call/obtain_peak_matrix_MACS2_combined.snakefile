...
Peak_OUTPUT_DIR = os.path.join(BOWTIE_DIR, 'Peaks/combined')
...
rule remove_blackList:
    input:
        peaks = os.path.join(PEAK_DIR, 'combined_peaks.narrowPeak')
    output:
        os.path.join(PEAK_DIR, 'combined_peaks.narrowPeak.clean.bed')
    params:
        blacklist = BLACKLIST
    shell:
        """
        cat {input} | cut -d'	' -f1-4 | bedtools intersect -v -a - -b {params} > {output}
        """

rule peak_IDs:
    input:
        peaks = os.path.join(PEAK_DIR, 'combined_peaks.narrowPeak.clean.bed')
    output:
        union = PEAK_UNION_fn
    shell:
        """
        cat {input.peaks} | cut -d'	' -f1-3 | awk "{{print \$0,"'"Peak"'"NR}}" | sort -k4,4 | sed "s/ /	/g  " > {output.union}
        """
